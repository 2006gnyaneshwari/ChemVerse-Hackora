...
def show_beaker(color, label, title="Experiment"):
    ...
    <div style="background: linear-gradient(to bottom, #8D6E63 0%, #6D4C41 100%); border-radius:15px; padding:20px; display:flex; flex-direction:column; align-items:center; box-shadow: inset 0 5px 15px rgba(0,0,0,0.5);">
        ...
        <div style="width:170px; height:230px; border:5px solid rgba(255,255,255,0.9); border-top:6px solid #ECEFF1; border-radius:0 0 40px 40px; position:relative; background: linear-gradient(to right, rgba(255,255,255,0.4) 0%, rgba(255,255,255,0.1) 20%, rgba(255,255,255,0.1) 80%, rgba(255,255,255,0.4) 100%); overflow:hidden; box-shadow: 0 15px 30px rgba(0,0,0,0.4);">
            <div style="position:absolute; bottom:0; width:100%; height:70%; background:{color}; border-radius:0 0 35px 35px; box-shadow: inset 0 15px 25px rgba(0,0,0,0.4);">
                <div style="position:absolute; top:-8px; width:100%; height:16px; background:{color}; border-radius:50%;"></div>
                <div style="position:absolute; width:14px; height:14px; background:rgba(255,255,255,0.8); border-radius:50%; bottom:20%; left:60%; animation: rise 2.2s infinite;"></div>
            </div>
        </div>
    ...
    <style>@keyframes rise {{0%{{transform:translateY(0); opacity:0.9;}}100%{{transform:translateY(-130px); opacity:0;}} }}</style>
    """
...
def show_volcano():
    ...
    <div style="background: linear-gradient(to bottom, #8D6E63 0%, #4E342E 100%); border-radius:15px; padding:20px; text-align:center;">
        ...
        <div style="color:#FFCC80; margin-top:10px; font-weight:bold;">Volcano Eruption</div>
    </div>
...
with main_tab1:
    ...
    if "Salt Analysis" in exp_type:
        salts_db = {
            "NaCl":{"name":"Common Salt","color":"#E8EAF6","obs":["White crystalline","No gas with dil acid","HCl fumes","Golden flame","AgNO3 white ppt"]},
            "Pb(NO3)2":{"name":"Lead Nitrate","color":"#E69A00","obs":["White turns yellow","White PbSO4","Brown NO2","White ppt soluble","Yellow PbI2"]},
            "CuSO4":{"name":"Copper Sulphate","color":"#0D47A1","obs":["Blue crystals","White BaSO4","No reaction","Pale blue Cu(OH)2","Chocolate brown"]},
            "FeCl3":{"name":"Ferric Chloride","color":"#4A0F0F","obs":["Dark brown","No gas","HCl gas","Brown Fe(OH)3","Blood red"]},
        }
        for ex in ["KCl","BaCl2","CaCO3","NiSO4","AgNO3","FeSO4"]:
        ...
        with c2:
            ...
            for i, tab in enumerate([t1,t2,t3,t4,t5]):
                with tab:
                    show_beaker(data["color"], f"{sel} - Exp {i+1}", f"Exp {i+1}")
                    st.markdown(f"<div class='observation-box'><b>Observation:</b> {data['obs'][i]}<br><b>Inference:</b> Note in lab manual</div>", unsafe_allow_html=True)

    elif "Litmus" in exp_type:
        st.subheader("Litmus Test")
        col1,col2=st.columns(2)
        with col1:
            liquid=st.selectbox("Select Liquid", ["Lemon Juice (Acid)","Soap Solution (Base)","Water (Neutral)","Vinegar (Acid)"])
        ...
        with col2:
            st.markdown(f"<div class='observation-box'><b>Blue Litmus:</b> {'Turns RED - Acid' if is_acid else 'No change'}<br><b>Red Litmus:</b> {'No change' if is_acid else 'Turns BLUE - Base' if 'Base' in liquid else 'No change'}<br><b>Result:</b> {liquid}</div>", unsafe_allow_html=True)

    elif "Copper Cycle" in exp_type:
        st.subheader("Copper Cycle - 4 Steps")
        step=st.slider("Select Cycle Step", 1,4,1)
        steps={1:("#B87333","Oxidation: 2Cu + O2 -> 2CuO (Black)"),2:("#212121","Precipitation: CuO + 2HCl -> CuCl2 (Blue)"),3:("#0D4F4F","Dissolution: CuCl2 + NaOH -> Cu(OH)2"),4:("#1565C0","Reduction: Cu(OH)2 -> Cu")}
        col,txt=steps[step]
        c1,c2=st.columns(2)
        with c1: show_beaker(col, f"Step {step}", f"Copper Cycle Step {step}")
        with c2: st.markdown(f"<div class='observation-box'><b>Reaction:</b> {txt}</div>", unsafe_allow_html=True)

    elif "Titration" in exp_type:
        st.subheader("Acid-Base Titration with Phenolphthalein")
        vol=st.slider("Add NaOH drops", 0, 50, 0)
        if vol<20:
            color="#FFCDD2"
            result="Acidic - Colorless"
        elif vol<25:
            color="#F8BBD0"
            result="Near Endpoint - Light Pink"
        else:
            color="#E91E63"
            result="Basic - Deep Pink"
        c1,c2=st.columns(2)
        with c1:
            show_beaker(color, f"{vol} ml NaOH", "Titration Flask")
        with c2:
            st.markdown(f"<div class='observation-box'><b>Indicator:</b> Phenolphthalein<br><b>Volume:</b> {vol} ml<br><b>Color:</b> {result}</div>", unsafe_allow_html=True)
            st.progress(vol/50)
        ...
        if st.button("ERUPT VOLCANO!", type="primary"):
            show_volcano()
            st.balloons()
            st.markdown("<div class='observation-box'><b>Reaction:</b> NaHCO3 + CH3COOH -> CO2 + H2O<br><b>Observation:</b> Foam erupts due to CO2 gas!</div>", unsafe_allow_html=True)
        else:
            show_volcano()

    elif "Cabbage" in exp_type:
        st.subheader("Cabbage Chemistry - Natural pH Indicator")
        ph=st.slider("pH of Solution", 1,14,7)
        if ph<3: color="#B71C1C"; label="pH 1-2 - Strong Acid - Red"
        elif ph<5: color="#E53935"; label="pH 3-4 - Acid"
        elif ph<7: color="#8E24AA"; label="pH 5-6 - Weak Acid"
        elif ph==7: color="#7B1FA2"; label="pH 7 - Neutral"
        elif ph<9: color="#1E88E5"; label="pH 8-9 - Base"
        elif ph<12: color="#43A047"; label="pH 10-11 - Base"
        else: color="#FDD835"; label="pH 12-14 - Strong Base"
        c1,c2=st.columns(2)
        with c1: show_beaker(color, label, "Cabbage Indicator")
        with c2: st.markdown(f"<div class='observation-box'><b>pH:</b> {ph}<br><b>Color:</b> {label}</div>", unsafe_allow_html=True)

    elif "Cartesian" in exp_type:
        st.subheader("Cartesian Diver - Buoyancy")
        ...
        <div style="background:linear-gradient(to bottom, #E1F5FE, #81D4FA); border-radius:15px; padding:20px; height:350px; position:relative; border:3px solid #0F5C5C;">
            <div style="position:absolute; bottom:{diver_pos}%; left:45%; width:30px; height:50px; background:#FF5722; border-radius:15px; transition:bottom 0.5s;"></div>
            <div style="position:absolute; bottom:0; width:100%; height:20px; background:#5D4037; left:0;"></div>
            <div style="color:#01579B; font-weight:bold; text-align:center;">Water Bottle - Squeeze to increase pressure</div>
        </div>
        """
        c1,c2=st.columns(2)
        with c1: components.html(html, height=400)
        with c2: st.markdown(f"<div class='observation-box'><b>Pressure:</b> {pressure}%<br><b>Diver:</b> {'Floating' if pressure<30 else 'Sinking' if pressure>70 else 'Middle'}</div>", unsafe_allow_html=True)
        ...
        <div style="background:#FFF8E1; border-radius:15px; padding:20px; text-align:center; border:3px solid #FF8F00;">
            <div style="width:10px; height:200px; background:#8D6E63; margin:0 auto; position:relative;">
                <div style="position:absolute; top:{200-size}px; width:{size}px; height:{size}px; background:#81D4FA; left:-{size//2}px; border-radius:5px;"></div>
            </div>
            <div style="width:120px; height:80px; background:#4FC3F7; margin:10px auto; border-radius:0 0 60px 60px;"></div>
            <div style="color:#E65100; font-weight:bold;">Day {days}</div>
        </div>
        """
        c1,c2=st.columns(2)
        with c1: components.html(html, height=380)
        with c2: st.markdown(f"<div class='observation-box'><b>Day:</b> {days}/14<br><b>Size:</b> {size}mm<br><b>Status:</b> {'No crystals' if days<2 else 'Growing' if days<10 else 'Ready!'}</div>", unsafe_allow_html=True)

    st.divider()
    st.subheader("Quick Save for Teacher")
    ...
    if tpw!="chem123" and is_teacher:
        ...
        if os.path.exists(DB_FILE):
            ...
            if df.empty: st.info("No data yet")
                ...
                c1.metric("Total", len(df)); c2.metric("Students", df['Roll'].nunique()); c3.metric("Popular", df['Experiment'].value_counts().index[0][:15])
                st.dataframe(df, use_container_width=True)
                st.bar_chart(df['Experiment'].value_counts())
                st.download_button("Download Report", df.to_csv(index=False), "ChemVerse_Report.csv")
        else:
            st.info("No data")

st.caption("ChemVerse v6.1 Fixed | IndentationError Solved | 8 Experiments")
